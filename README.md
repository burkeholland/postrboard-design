...
Install the skill by copying [`skill/postrboard/`](skill/postrboard/) into the agent's skills directory (for example `~/.agents/skills/postrboard/`). Prefer package CSS (`node_modules/postrboard-css/postrboard.min.css`) over the unversioned CDN so the skill, registry, and stylesheet stay on the same release.

The skill tells an agent how to:

- Start from who uses the screen and what they are looking at.
- Open the `components.md` index, jump to each component by name, and paste the markup instead of guessing class names.
