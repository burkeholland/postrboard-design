...
- **Preview / Markup switch in the documentation** - Each component demo now has a two-way switch built from the framework's own `segmented-control`, so the page dogfoods a real component. It is pure CSS, using radio inputs and `:has()`, and adds no JavaScript. The markup is hidden until asked for, which cut the components section from 46,714px to 26,746px
