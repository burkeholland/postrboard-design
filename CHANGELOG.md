...
- `.terminal-bar` rendered three literal macOS traffic lights — the "Fake technicality" tell from the framework's own guardrail table; the bar is now a mono label slot (`.terminal-title`) and the dots are neutral
...
- Dashboard examples and docs demos padded single-digit counts to `08`, `00`, `07`, and `09`, and the publishing example numbered its cards `01 /` … `05 /` out of order — both are the "Generator scaffolding" tell the guardrail table warns about; counts are now real and the kickers are plain editorial categories
